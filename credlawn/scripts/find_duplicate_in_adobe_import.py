...
def update_file_type():
    # Get all records from the Adobe Import doctype
    records = frappe.get_all('Adobe Import', fields=['name', 'duplicate_finder', 'dap_final_flag', 'promo_code'])

    # Set default file_type to "Not Set" for all records initially
    for record in records:
        frappe.db.set_value('Adobe Import', record['name'], 'file_type', 'Not Set')
    ...
    for df_value, group in grouped_records.items():
        ...
        for record in group:
            if record['dap_final_flag'] is not None and record['promo_code'] is not None:
                frappe.db.set_value('Adobe Import', record['name'], 'file_type', 'New')
                new_entry_found = True
                # Mark all others as Duplicate
                for r in group:
                    if r['name'] != record['name']:
                        frappe.db.set_value('Adobe Import', r['name'], 'file_type', 'Duplicate')
        ...
        if not new_entry_found:
            ...
            frappe.db.set_value('Adobe Import', group[0]['name'], 'file_type', 'New')  # Set the first as New
            for r in group[1:]:
                frappe.db.set_value('Adobe Import', r['name'], 'file_type', 'Duplicate')
